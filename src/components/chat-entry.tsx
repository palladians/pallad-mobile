...
const ChatPayment = ({ message }: { message: Message }) => {
	...
	return (
		<Link href={`/inbox/transaction/${message.hash}`} asChild>
			<Pressable
				className={clsx(
					"flex flex-col gap-2 mx-4 border-2 p-4 rounded-2xl mb-2",
					outgoing
						? "self-end items-end border-brand"
						: "items-start self-start border-gray-600",
				)}
			>
				<HStack className="items-center gap-1">
					{outgoing ? (
						<ArrowLeftIcon color={colors.zinc["400"]} width={12} height={12} />
					) : (
						<ArrowRightIcon color={colors.zinc["400"]} width={12} height={12} />
					)}
